...
function scheduleModify_en() {
    ...
    if(isArray_en("index",msgArray) == 1)
        indexAll = 1;
    else
        indexAll = 2;
     ...
     // 提示
         scheduleModifyTooip = myApp.toast.create({text: "Please enter the scenario name", position: 'center', closeTimeout: 2000, });
         scheduleModifySuccessTooip = myApp.toast.create({text: "Save successfully", position: 'center', closeTimeout: 2000, });
         scheduleModifyInputTooip = myApp.toast.create({text: "Scenario names cannot be repeated", position: 'center', closeTimeout: 2000, });
}
    ...
    $.when(AlarmCenterContext.post("/api/GWServiceWebAPI/get_AdministratorData", {getDataTable: "0"}),AlarmCenterContext.post("/api/GWServiceWebAPI/get_EquipGroupData", {getDataTable: "0"})).done(function(n,l){
         ...
         if(n_result.code == 200 && l_result.code == 200)
         {
            ...
            n_result.data.forEach(function(item,index){
                if(indexAll == 1)
                {
                   if(isArray_en("username",msgArray) == item.Administrator)
            ...
            l_result.data.forEach(function(item,index){
                if(indexAll == 1)
                {
                    if(isArray_en("groupname",msgArray) == item.group_name)
                   ...
                   htmlContent +=`<li class="swipeout bottomBorderLine" equipcomb="${valueArray[i]}">
                        <div class="item-content swipeout-content schedule-content row no-gap" >
                            <div class="item-inner">
                              <div class="item-title">${i+1}、${(equip_no_flg?filterFun_en(equipList,valueArray[i].split(",")[0],null):(valueArray[i]?"Interval operation":""))}: <strong>${(equip_no_flg?filterFun_en(setList,valueArray[i].split(",")[0],valueArray[i].split(",")[1]):(valueArray[i]?"Delay interval"+valueArray[i]+"Millisecond":""))}</strong></div>
                              <div class="item-after" onclick="scenalControlPro_en(this)" index="${i}"><i class="iconfont icon-f7_top_jt"></i></div>
                            </div>
                        </div>
                        <div class="swipeout-actions-right">
                          <a href="#" class="delBtn" onclick="currentControl_en(this)" style="">Delete</a>
                        </div>
                      </li> `;
                  }
...
function submitScene_en(dt) {
    if(indexAll == 1)
    {
          ...
          let reqData = {equipNo: isArray_en("equip_no",msgArray), setNo: isArray_en("set_no",msgArray), sceneName: sceneName, dataStr: dataStr }
    ...
    $.when(AlarmCenterContext.post("/api/GWServiceWebAPI/getSetparmList",{findEquip:false})).done(function(l){
        ...
        if (l.HttpData.code ==200) {
            ...
            equipRt.data.forEach(function(item,index){
                if(indexAll == 1)
                {
                   if(isArray_en("username",msgArray) == item.set_nm && isArray_en("equip_no",msgArray) == item.equip_no)
