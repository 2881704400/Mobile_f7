...
function Message() {
    // 主页面下拉刷新
    $$('#message_pg_content').on("ptr:refresh", refreshpg_ms);

    loadMessage();
    toolbarActive('MessageTool');
}

function refreshpg_ms(e){
    setTimeout(function () {
        initEnsureChonglian(function () {
            loadMessage();
        });
        $("#message_pg_content").removeClass("ptr-pull-up ptr-transitioning ptr-refreshing");
...
myApp.on("pageInit",  function (page) {
    setTimeout(function () {
        ...
        if (page.view.history[MessageListindex].split("?")[0] == "/MessageList/") {
            ...
            onSnapshot(pages);
            $$('#MessageList_ref').on("ptr:refresh", refreshMList_ms);
        }
    }, 500);
});

function refreshMList_ms(e) {
    setTimeout(function () {
        initEnsureChonglian(function () {
            SnapshotData();
        });
        myApp.ptr.done();// 加载完毕需要重置
        $("#MessageList_ref").removeClass("ptr-pull-up ptr-transitioning ptr-refreshing");
    }, 2000);
}
